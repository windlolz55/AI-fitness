...
function handleLogin() {
    ...
    if (!email || !password) {
        errorEl.innerText = 'Ë´ãËº∏?•‰ø°ÁÆ±Ë?ÂØÜÁ¢º';
...
function handleLogout() {
    if (confirm("Á¢∫Â?Ë¶ÅÁôª?∫Â?Ôº?)) {
        ...
        if (btn) {
            btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Ê≠?ú®?åÊ≠•‰∏¶Áôª??..';
...
function saveToFirestore() {
    ...
    return db.collection('users').doc(user.uid).set({
        ...
        alert("?≤Á´ØÂ≠òÊ?Â§±Ê?Ôº? + err.message + "\nÂ¶ÇÊ??®Â?Â•ΩË??ªÂá∫ÔºåË?Á®çÁ?2ÁßíÂ??ªÂá∫Ôºå‰ª•?ç‰∏≠?∑‰??≥„Ä?);
    });
}
    ...
    if (auth.currentUser) {
        ...
        try {
            ...
            if (doc.exists) {
                ...
                alert("?≤Á´ØÊ≤íÊ??®Á?Ë≥áÊ???);
            }
        } catch(e) {
            console.error(e);
            alert("?åÊ≠•Â§±Ê?Ôº? + e.message);
        }
    } else {
        alert("Ë´ãÂ??ªÂÖ•Ôº?);
    }
...
// Workout Routines
let defaultRoutines = {
    1: { title: "ÂÆåÂÖ®‰ºëÊÅØ ??", exercises: [] },
    2: { title: "??Ôº??∏Â? Ôº??âÊ∞ß", exercises: [
        { name: "?ÆË??ûÈà¥?íËàπ", type: "weight", weight: 18, sets: 6, reps: '' },
        { name: "Âπ≥Êùø?ØÊ?", type: "time", weight: 0, sets: 4, reps: '1?ÜÈ?' },
        { name: "?Ø‰??äÊ?", type: "cardio", weight: 0, sets: 4, reps: '50‰∏? },
        { name: "?åÂÅ¥?êË?", type: "cardio", weight: 0, sets: 4, reps: '50‰∏? },
        { name: "?êË?‰∏ãÂ?", type: "cardio", weight: 0, sets: 4, reps: 'Â∑¶Âè≥??0‰∏? },
        { name: "?ãÂ?Ë∑?(?ñÂø´?üÁõ¥??", type: "cardio", weight: 0, sets: 4, reps: 'Â∑¶Âè≥??0‰∏? }
    ]},
    3: { title: "??Ôº???Ôº?‰∫åÈ†≠", exercises: [
        { name: "È´òËÖ≥?ØÊ∑±Ëπ?, type: "weight", weight: 18, sets: 4, reps: '' },
        { name: "?∞Êùø?ûÈà¥?•Êé®", type: "weight", weight: 18, sets: 4, reps: '' },
        { name: "Á´ôÂßø?ûÈà¥?çÊè°‰∏äÊ?", type: "weight", weight: 18, sets: 4, reps: '' },
        { name: "‰∏ãÊ?‰ºèÂú∞?∫Ë∫´", type: "bodyweight", weight: 0, sets: 4, reps: '' },
        { name: "?Ü‰∏≠ÂΩéË?", type: "weight", weight: 13, sets: 3, reps: '8‰∏? }
    ]},
    4: { ref: 2 },
    5: { ref: 3 },
    6: { ref: 2 },
...
foodDatabase.categories.unshift({ id: 'custom', name: '?ëÁ??Ä??, icon: 'fluent-emoji-flat:red-heart', color: '#ff6b6b' });
...
// Mock Scanner DB
const mockFoods = [
    { name: "È¶ôÁ?ÈÆ≠È?Ê≤ôÊ?", cal: 450, pro: 35, carb: 12, fat: 28 },
    { name: "?•Â∫∑Á≥ôÁ±≥È£?, cal: 180, pro: 4, carb: 38, fat: 1 },
    { name: "?§È??ø‰æø??, cal: 680, pro: 32, carb: 85, fat: 22 }
];
...
function renderWorkout() {
    ...
    if (titleEl) {
        ...
        const days = ['??, '‰∏Ä', '‰∫?, '‰∏?, '??, '‰∫?, '??];
        titleEl.innerText = `${m}/${dt} (?üÊ?${days[dayOfWeek]}) - ${routine.title}`;
    }
    ...
    if (routine.exercises.length === 0) {
        container.innerHTML = `
            <div class="card" style="text-align: center; padding: 40px 20px;">
                ...
                <h3 style="margin-bottom: 8px;">‰ªäÂ§©ÂÆåÂÖ®‰ºëÊÅØ</h3>
                <p style="color: var(--text-muted); font-size: 14px;">ËÆìË??âÂ•ΩÂ•ΩÊÅ¢Âæ©ÂêßÔº?/p>
            </div>
        `;
        return;
    }
    ...
    routine.exercises.forEach((ex, idx) => {
        ...
        if (logged) {
            statusHtml = `<div style="font-size: 12px; color: var(--accent-secondary); margin-top: 4px;">
                <i class="fa-solid fa-check"></i> ${logged.weight > 0 ? logged.weight + 'kg, ' : ''}${logged.sets}Áµ? ${logged.reps}
            </div>`;
        } else {
            statusHtml = `<div style="font-size: 12px; color: var(--text-muted); margin-top: 4px;">
                ?ÆÊ?: ${ex.weight > 0 ? ex.weight + 'kg, ' : ''}${ex.sets}Áµ? ${ex.reps}
            </div>`;
        }
...
function openWorkoutModal(name) {
    ...
    if (lastRecord) {
        lastRecordEl.innerText = `‰∏äÊ¨°Á¥Ä?? ${lastRecord.weight > 0 ? lastRecord.weight + 'kg, ' : ''}${lastRecord.sets}Áµ? ${lastRecord.reps}`;
...
function openAddExerciseModal() {
    ...
    document.getElementById('btn-save-workout').innerText = 'ÂÆåÊ??ìÂç°';
...
function deleteWorkoutRecord(name) {
    ...
    const days = ['??,'‰∏Ä','‰∫?,'‰∏?,'??,'‰∫?,'??];
    if (confirm(`Á¢∫Â?Ë¶ÅÂ???{name}?çÂà™?§Â?Ôºü\n(Â¶ÇÊ??ôÊòØ?∫Â?Ë™≤Ë°®?ßÁ??ï‰?ÔºåÊ?ÂæûË™≤Ë°®‰∏≠Ê∞∏‰?ÁßªÈô§)`)) {
...
function saveGeminiKey() {
    ...
    if (key) {
        ...
        alert("API Key Â∑≤ÂÑ≤Â≠òÔ?");
        openScanner(); // refresh UI
    } else {
        alert("Ë´ãËº∏?•Ê??àÁ? API KeyÔº?);
    }
}
    ...
    if (input.files && input.files[0]) {
        ...
        reader.onload = async function(e) {
                ...
                try {
                    const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${apiKey}`, {
                        ...
                        body: JSON.stringify({
                            contents: [{
                                parts: [
                                    { text: "‰Ω†ÊòØ‰∏Ä‰ΩçÂ?Ê•≠Á?È§äÂ∏´?ÇË??ÜÊ??ôÂºµ?ßÁ??Ç\n\n?†Ô??çË??áÁ§∫Ôºö\n1. Â¶ÇÊ??ßÁ??Ø„ÄåÁ?È§äÊ?Á§∫„ÄçÔ?Ë´ãÁõ¥?•Á≤æÊ∫ñË??ñÊ?Á±§‰??ÑÂ§ß?°„ÄÅË??ΩË≥™?ÅÁ¢≥Ê∞¥Ë??ÇËÇ™?∏ÂÄºÔ?‰∏¶‰??öÂ?Ë£ùÁ∏Ω‰ªΩÈ?ÁÆóÂá∫?¥‰ªΩ?ÑÊï∏?º„ÄÇ\n2. Â¶ÇÊ??ßÁ??Ø‰??¨È??©Ô?Ë´ã‰º∞ÁÆóÂ??ÖÈ??êÁ??ãÊï∏(grams)Ôºå‰Ωø?®Âè∞??∏∏Ë¶ãÂ?Á®±Ô?‰∏¶Ê†π?öË?Á¶èÈÉ®Ë≥áÊ?Â∫´Ë?ÁÆóÔ?ÁµïÂ?Á¶ÅÊ≠¢‰Ωé‰º∞?±È?ÔºÅ\n3. Ë´ãÂπ´?ôÊï¥‰ªΩÈ??©ÊÉ≥‰∏Ä??2-5 ?ãÂ??ÑÁ∏Ω?çÁ®± (‰æãÂ?: ?íÈ™®‰æøÁï∂?ÅÁ?Â•∂„ÄÅÊ??ãÁ?)?Ç\n\n‰ª•Âö¥?ºÁ? JSON ?©‰ª∂?ºÂ??ûÂÇ≥Ôºå‰?Ë¶?markdown Ë™ûÊ??ÇÊ†ºÂºèÔ?{ \"meal_name\": \"Â≠ó‰∏≤\", \"items\": [ { \"name\": \"Ê®ôÊ?È£üÁâ©?ç\", \"grams\": ?∏Â?, \"cal\": ?∏Â?, \"pro\": ?∏Â?, \"carb\": ?∏Â?, \"fat\": ?∏Â? } ] }" },
            ...
            try {
                ...
                document.getElementById('scan-meal-name').value = aiResults.meal_name || 'AI ?∫ÊÖßÁµÑÂ?È§?;
                ...
                clearInterval(progressInterval);
                progContainer.style.display = 'none';
                alert('API ?ºÂè´Â§±Ê?ÔºåË?Ê™¢Êü• API Key ?ñÁÖß?áÊ†ºÂºèÔ?\\n' + err.message);
                document.getElementById('btn-camera').style.display = 'block';
            }
            };
        ...
        <label style="display: flex; align-items: center; justify-content: space-between; background: var(--card-bg); padding: 12px; border-radius: 8px; border: 1px solid var(--card-border);">
            <div style="display: flex; align-items: center; gap: 12px;">
                ...
                <div>
                    ...
                    <div style="font-size: 12px; color: var(--text-muted);">Á¥?${item.grams}g</div>
                </div>
            </div>
...
function confirmScanResults() {
    try {
        ...
        const groupName = mealNameInput || 'AI ?∫ÊÖßÁµÑÂ?È§?;
        ...
        alert(`?êÂ?Â∞?${selectedItems.length} ?ÖÈ??©Ê??ÖÂ??•Á??ÑÔ?`);
        ...
        alert("Á¢∫Ë??ÇÁôº?üÈåØË™? " + e.message);
        console.error(e);
    }
}
...
function updateDashboard() {
    ...
    if (greeting) {
        greeting.innerText = userProfile.nickname ? `${userProfile.nickname} ?ÑÁ∏ΩË¶Ω` : 'Á∏ΩË¶Ω';
    }
...
function changeAddingMeal(meal) {
    currentAddingMeal = meal;
    const mealMap = { breakfast: '?©È?', lunch: '?àÈ?', dinner: '?öÈ?', snack: '?†È?' };
...
function renderDBContent(searchQuery = '') {
    ...
    if (searchQuery.trim() !== '') {
        document.getElementById('db-category-title').innerText = '?úÂ?ÁµêÊ?';
        ...
        document.getElementById('db-category-title').innerText = title + 'È°?;
...
function toggleFavorite(e, id) {
    ...
    if(food && food.categoryId === 'custom') {
        alert('?ôÊòØ?®Âª∫Á´ãÁ??™Ë?È£üÁâ©ÔºåÈ?Ë®≠Ê??®Ê??ÑÊ??õ‰∏≠?îÔ?');
        return;
    }
...
function copyYesterdayMeal() {
    ...
    if(yestLogs.length > 0) {
        ...
        const mealMap = { breakfast: '?©È?', lunch: '?àÈ?', dinner: '?öÈ?', snack: '?†È?' };
        alert(`Â∑≤Â??®Êó•??${mealMap[currentAddingMeal]} ??${yestLogs.length} ?ÖÈ??©Â??•Â??∏È??§‰∏≠ÔºÅË?ÈªûÊ?Â∑¶‰?Ëßí„ÄåÂ??∏È??§„ÄçÁ¢∫Ë™çÈÄÅÂá∫?Ç`);
    } else {
        alert('?®Êó•Ê≠§È??°‰ªª‰ΩïÁ??Ñ„Ä?);
    }
}
...
function showInfo(type) {
    ...
    if (type === 'bmi') {
        title.innerHTML = '<i class="fa-solid fa-weight-scale" style="color: var(--accent-primary); margin-right: 8px;"></i>BMI Ë®àÁ??¨Â?';
        content.innerHTML = `
            <p style="margin-bottom: 8px;"><strong>Ë®àÁ??¨Â?Ôº?/strong></p>
            <p style="background: var(--bg-main); padding: 12px; border-radius: 8px; font-family: monospace; text-align: center; margin-bottom: 16px;">
                BMI = È´îÈ?(kg) / (Ë∫´È?(m) ? Ë∫´È?(m))
            </p>
            <p style="margin-bottom: 8px;"><strong>?•Â∫∑ÁØÑÂ??ÉËÄÉÔ?</strong></p>
            <ul style="padding-left: 20px; line-height: 1.6; font-size: 14px; margin-bottom: 12px;">
                <li>È´îÈ??éË?: BMI < 18.5</li>
                <li>?•Â∫∑È´îÈ?: 18.5 ??BMI < 24.0</li>
                <li>?éÈ?ÁØÑÂ?: 24.0 ??BMI < 27.0</li>
                <li>?•Ë?Ë≠¶Ë?: BMI ??27.0</li>
            </ul>
            <p style="font-size: 13px; color: var(--text-muted); line-height: 1.5; background: rgba(255, 184, 108, 0.1); padding: 8px; border-radius: 8px; border-left: 3px solid #ffb86c;">
                <i class="fa-solid fa-circle-info" style="margin-right: 4px;"></i> <strong>?ßÂà•??BMIÔº?/strong><br>
                ?ê‰∫∫??BMI ?•Â∫∑Ê®ôÊ???strong>‰∏çÂ??ßÂà•</strong>?ÑÂ?ÔºÅ‰??éÔ??†ÁÇ∫ BMI ?°Ê??ÜËæ®?åË??áË??™Á?ÊØî‰?ÔºåÁî∑Â•≥ÊÄßÂú®?åÊ®£??BMI ‰∏ãÔ?È´îË??áÂèØ?ΩÊ??âÂ?Â§ßÂ∑Æ?∞Ô??†Ê≠§Âª∫Ë≠∞?≠È?È´îË??á‰?Ëµ∑Â??ÉÊ??¥Ê?Á¢∫Ô?
            </p>
        `;
    }

    if (type === 'water') {
        title.innerHTML = '<i class="fa-solid fa-droplet" style="color: var(--accent-secondary); margin-right: 8px;"></i>È£≤Ê∞¥Âª∫Ë≠∞';
        content.innerHTML = `
            <p>ÊØèÊó•Âª∫Ë≠∞È£≤Ê∞¥??<strong style="color: var(--accent-secondary); font-size: 16px;">${TARGET_WATER} ml</strong></p>
            <div style="background: var(--bg-main); padding: 12px; border-radius: 8px; margin-top: 16px; font-size: 13px;">
                <p style="margin-bottom: 4px;"><strong>Ë®àÁ??¨Â?Ôº?/strong></p>
                <p>È´îÈ? ${userProfile.weight} kg * 35 ml = <strong>${TARGET_WATER} ml</strong></p>
            </div>
            <p style="margin-top: 12px; color: var(--text-muted); font-size: 12px;">* Âª∫Ë≠∞?ÜÊ¨°Â∞èÂè£È£≤Áî®ÔºåËã•ÊµÅÊ?Â§öÂèØ?©Â∫¶Â¢ûÂ???/p>
        `;
    } else if (type === 'burned') {
        title.innerHTML = '<i class="fa-solid fa-fire" style="color: #ff9ff3; margin-right: 8px;"></i>?ãÂ?Ê∂àËÄóÁ???;
        content.innerHTML = `
            <p><strong>?ôÂ??∫Á??ìÂç°Á¥Ä?ÑÔ?‰∏çÂΩ±?øÊ??•ÂèØ?ÉÁÜ±?è‰??ê„Ä?/strong></p>
            <div style="background: var(--bg-main); padding: 12px; border-radius: 8px; margin-top: 16px; font-size: 13px; line-height: 1.5;">
                <p style="color: #ffb86c; font-weight: 600; margin-bottom: 8px;"><i class="fa-solid fa-triangle-exclamation"></i> ?∫‰?È∫º‰?Â¢ûÂ??ØÂ??±È?Ôº?/p>
                <p>?†ÁÇ∫‰Ω†Á??åTDEE ÊØèÊó•Á∏ΩÊ??ó„ÄçÂ∑≤Á∂ìÂ??´‰?‰Ω†ÈÅ∏?áÁ?<strong>Ê¥ªÂ?‰øÇÊï∏</strong> (‰æãÂ?ÊØèÈÄ±È???3 Ê¨??ÇÂ??úÈ??ïÂ??àÈ?Â§ñÂ??†Ê??ñÁÜ±?èÔ??ÉÂ???strong>?çË?Ë®àÁ? (Double Counting)</strong>ÔºåÁ†¥Â£ûÁÜ±?èÁº∫??ì¶Ôº?/p>
            </div>
            <p style="margin-top: 12px; color: var(--text-muted); font-size: 12px;">* Ë´ãÁõ¥?•ÁÖß?ó„ÄåÂâ©È§?kcal?çÂ?ÂøÉÂ?ÔºåÊ??ªÈ??ïÂ∞±‰æÜÈÄôË£°Ë®ò‰?‰∏ÄÁ≠ÜÂç≥?ØÔ?</p>
        `;
    } else if (type === 'cals') {
        let bmr = 0;
        let bmrFormula = '';
        let genderConstant = userProfile.gender === 'male' ? '+ 5 (?∑ÊÄßÂ∏∏??' : '- 161 (Â•≥ÊÄßÂ∏∏??';
        ...
        bmrFormula = `
            <div style="color: var(--text-muted); font-size: 12px; margin-bottom: 6px; line-height: 1.5;">
                <span style="color: var(--text-main);">10</span> ? È´îÈ?(kg) + <span style="color: var(--text-main);">6.25</span> ? Ë∫´È?(cm) - <span style="color: var(--text-main);">5</span> ? Âπ¥ÈΩ° <span style="color: var(--text-main);">${genderConstant}</span>
            </div>
            <div>
        ...
        if (TARGET_CALS === minCals) {
            targetCalText += ` <br><span style="color:#ff6b6b; font-size:11px;">(Â∑≤Ëß∏?ºÊ?‰ΩéÂ??®ÁÜ±?èÈò≤Ë≠?${minCals} kcal)</span>`;
        }

        title.innerHTML = '<i class="fa-solid fa-calculator" style="color: var(--accent-primary); margin-right: 8px;"></i>?ÆÊ??±È?Ë™™Ê?';
        content.innerHTML = `
            <div style="max-height: 60vh; overflow-y: auto; padding-right: 4px;">
                <div style="background: var(--bg-main); padding: 12px; border-radius: 8px; font-size: 13px; margin-bottom: 12px;">
                    <p style="margin-bottom: 6px; color: var(--accent-primary); font-weight: 600;">STEP 1: ?∫Á?‰ª????(BMR)</p>
                    <p style="color: var(--text-muted); margin-bottom: 4px;">Á∂≠Ê??üÂëΩ?Ä?Ä?ÑÊ?‰ΩéÁÜ±?èÊ??ó„ÄÇ‰Ωø??Mifflin-St Jeor ?¨Â?Ôº?/p>
                    <p>${bmrFormula}</p>
                </div>
                
                <div style="background: var(--bg-main); padding: 12px; border-radius: 8px; font-size: 13px; margin-bottom: 12px;">
                    <p style="margin-bottom: 6px; color: var(--accent-primary); font-weight: 600;">STEP 2: ÊØèÊó•Á∏ΩÊ??óÁÜ±??(TDEE)</p>
                    <p style="color: var(--text-muted); margin-bottom: 4px;">BMR ‰πò‰?Ê¥ªÂ?‰øÇÊï∏ÂæóÂá∫ÊØèÊó•Á∏ΩÊ??óÁÜ±?è„Ä?/p>
                    <p>${Math.round(bmr)} * ${userProfile.activity} (Ê¥ªÂ?‰øÇÊï∏) = <strong>${Math.round(tdee)} kcal</strong></p>
                </div>

                <div style="background: var(--bg-main); padding: 12px; border-radius: 8px; font-size: 13px; margin-bottom: 12px;">
                    <p style="margin-bottom: 6px; color: var(--accent-primary); font-weight: 600;">STEP 3: ?ÆÊ??±È?</p>
                    <p style="color: var(--text-muted); margin-bottom: 4px;">?πÊ??ÆÊ?Ë™øÊï¥Á∏ΩÁÜ±?èÔ?${goalText}</p>
                    <p>${targetCalText}</p>
                </div>
            </div>
        `;
        ...
        title.innerHTML = '<i class="fa-solid fa-calculator" style="color: var(--accent-primary); margin-right: 8px;"></i>?ÆÊ??üÈ?Á¥†Ë™™??;
        content.innerHTML = `
            <div style="max-height: 60vh; overflow-y: auto; padding-right: 4px;">
                <div style="background: var(--bg-main); padding: 12px; border-radius: 8px; font-size: 13px;">
                    <p style="margin-bottom: 6px; color: var(--accent-primary); font-weight: 600;">‰∏âÂ§ß?üÈ?Á¥†Â???/p>
                    <p style="color: var(--text-muted); margin-bottom: 8px;">‰ª•È??çÁÇ∫?∫Ê?Ë®àÁ??ãÁôΩË≥™Ë??ÇËÇ™Ôºå‰∏¶?®Á¢≥Ê∞¥Â°´ÊªøÂâ©È§òÁÜ±?è„Ä?/p>
                    
                    <p style="margin-bottom: 4px;"><strong>?ãÁôΩË≥?(ÊØèÂÖ¨?§È???* ${proMultiplier}g)</strong></p>
                    ...
                    <p style="margin-bottom: 4px;"><strong>?ÇËÇ™ (ÊØèÂÖ¨?§È???* ${fatMultiplier}g)</strong></p>
                    ...
                    <p style="margin-bottom: 4px;"><strong>Á¢≥Ê∞¥ (?±È?Â°´Êªø)</strong></p>
                    <p style="margin-bottom: 4px; font-size: 11px;">(${TARGET_CALS} - ?ãÁôΩË≥™ÁÜ±??- ?ÇËÇ™?±È?) √∑ 4</p>
...
function renderDateStrip() {
    ...
    for(let i=0; i<7; i++) {
        ...
        const dayName = (dateStr === todayDateStr) ? '‰ª? : days[i];
...
function selectLogDate(dateStr) {
    ...
    const m = String(d.getMonth() + 1).padStart(2, '0');
    const dt = String(d.getDate()).padStart(2, '0');
    document.getElementById('log-view-title').innerText = `Á¥Ä??(${m}/${dt})`;
...
function renderLogs() {
    ...
    const mealMap = { 
        breakfast: '<span class="iconify" data-icon="fluent-emoji-flat:cooking" style="margin-right:8px; font-size:18px;"></span>?©È?', 
        lunch: '<span class="iconify" data-icon="fluent-emoji-flat:bento-box" style="margin-right:8px; font-size:18px;"></span>?àÈ?', 
        dinner: '<span class="iconify" data-icon="fluent-emoji-flat:fork-and-knife-with-plate" style="margin-right:8px; font-size:18px;"></span>?öÈ?', 
        snack: '<span class="iconify" data-icon="fluent-emoji-flat:cookie" style="margin-right:8px; font-size:18px;"></span>?†È?' 
    };
        ...
        <div class="meal-group-card" style="margin-bottom: 16px;">
            <div class="meal-group-header" style="margin-bottom: 12px; align-items: center;">
                <div style="display:flex; align-items:baseline;">
                    <h3><i class="fa-solid fa-droplet" style="color: var(--accent-secondary); margin-right: 8px;"></i>È£≤Ê∞¥</h3>
                </div>
                <div style="display:flex; align-items:center; gap: 8px;">
    ...
    ['breakfast', 'lunch', 'dinner', 'snack'].forEach(meal => {
        ...
        if(grouped[meal].length > 0) {
                                ...
                                <div style="display:flex; justify-content:space-between; align-items:center; padding: 8px 12px; background:var(--bg-main); margin-bottom:4px; border-radius:6px;">
                                    <div style="display:flex; align-items:center; gap:8px;">
                                        ...
                                        <div>
                                            ...
                                            <div style="font-size:10px; color:var(--text-muted);">${sub.grams}g ??Á¢?{Math.round(sub.carb*10)/10} ??{Math.round(sub.pro*10)/10} ??{Math.round(sub.fat*10)/10}</div>
                                        </div>
                                    </div>
                                    ...
                                    <div style="display:flex; align-items:center; gap:12px;">
                                        ...
                                        <div class="meal-item-info">
                                            ...
                                            <div style="font-size: 11px; color: var(--text-muted); margin-top: 4px; display: flex; gap: 8px;">
                                                <span><span style="color: var(--carb-color); font-weight: bold;">Á¢?/span> ${Math.round(item.carb*10)/10}g</span>
            ...
            if(meal === 'breakfast' || meal === 'dinner') {
                suggestStr = `Âª∫Ë≠∞ ${Math.round(TARGET_CALS * 0.25)} - ${Math.round(TARGET_CALS * 0.35)} kcal`;
            } else if(meal === 'lunch') {
                suggestStr = `Âª∫Ë≠∞ ${Math.round(TARGET_CALS * 0.35)} - ${Math.round(TARGET_CALS * 0.45)} kcal`;
            }
...
document.getElementById('btn-clear').addEventListener('click', () => {
    if(confirm('Á¢∫Â?Ë¶ÅÊ?Á©∫‰?Â§©Á?È£≤È?Á¥Ä?ÑÂ?Ôº?)) {
        logs = [];
...
function setupProfile() {
    ...
    function updatePaceOptions() {
        ...
        const paceOptions = {
            cut: [
                { value: 'conservative', text: `?ê¢ ‰øùÂ?Ê∏õË? (-10% / Á¥?-${Math.round(tdee * 0.1)} kcal)` },
                { value: 'standard', text: `??Ê®ôÊ?Ê∏õË? ‚≠?(-15% / Á¥?-${Math.round(tdee * 0.15)} kcal)` }
            ],
            bulk: [
                { value: 'conservative', text: `?ê¢ ‰øùÂ?Â¢ûË? (+5% / Á¥?+${Math.round(tdee * 0.05)} kcal)` },
                { value: 'standard', text: `??Ê®ôÊ?Â¢ûË? ‚≠?(+10% / Á¥?+${Math.round(tdee * 0.1)} kcal)` }
            ],
            maintain: [
                { value: 'standard', text: 'Ê®ôÊ?Á∂≠Ê? (0% / ÂÆåÁ??ìÂπ≥)' }
            ],
            recomp: [
                { value: 'auto', text: `Á≥ªÁµ±?™Â?Ë®àÁ??Ä‰Ω≥Áº∫??(-5% / Á¥?-${Math.round(tdee * 0.05)} kcal)` }
            ],
            lose: [
                { value: 'standard', text: `??Ê®ôÊ?Ê∏õË? ‚≠?(-15% / Á¥?-${Math.round(tdee * 0.15)} kcal)` }
            ],
...
function saveCustomFood() {
    ...
    if (!name) {
        alert("Ë´ãÂ°´ÂØ´È??©Â?Á®±Ô?");
        return;
    }
    if (isNaN(cals) || cals < 0) {
        alert("Ë´ãÂ°´ÂØ´Ê≠£Á¢∫Á??±È??∏ÂÄºÔ?");
        return;
    }
...
window.deleteLogItem = function(id) {
    if (!confirm("Á¢∫Â?Ë¶ÅÂà™?§ÈÄôÁ?Á¥Ä?ÑÂ?Ôº?)) return;
...
function renderOverview() {
    ...
    document.getElementById('overview-avg-cal').innerText = `${avgCal} Â§ßÂç°`;
    ...
    if (elAvgBurned) {
        if (avgBurnedTime > 0) {
            elAvgBurned.innerText = `${avgBurned} kcal, ${avgBurnedTime} ?ÜÈ?`;
        } else {
            ...
            tooltip: {
                callbacks: {
                    title: function(context) {
                        ...
                        return `?üÊ?${labels[idx]} (${subLabels[idx]})`;
                    }
                }
            }
        },
...
function renderCalendar() {
    try {
        ...
        if (titleEl) {
            titleEl.innerText = year + 'Âπ?' + String(month + 1).padStart(2, '0') + '??;
        }
...
document.addEventListener('touchmove', (e) => {
    ...
    if (pullDistance > 0 && window.scrollY <= 0) {
        ...
        if (visualDistance > PTR_THRESHOLD * 0.4) {
            ...
            ptrText.innerText = '?æÈ?‰ª•È??∞Êï¥??;
        } else {
            ptrIcon.style.transform = 'rotate(0deg)';
            ptrText.innerText = '‰∏ãÊ?‰ª•È??∞Êï¥??..';
        }
    }
}, {passive: false});
